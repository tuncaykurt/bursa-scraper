...
async def trigger_scrape_post(request: ScrapeRequest):
    """
    POST endpoint to trigger Bursa scraping (all listings, no filters)
    """
    proxy_dict = None
    if request.proxy:
...
async def trigger_scrape_logic(limit: int = None, proxy: dict = None):
    ...
    Scrapes ALL listings from Bursa (no filters)
    """
    try:
        ...
        logger.info(f"Webhook received - scraping {limit} listings from BURSA (all listings){proxy_info}")
        
        listings = await run_scraper(limit=limit, proxy=proxy)
